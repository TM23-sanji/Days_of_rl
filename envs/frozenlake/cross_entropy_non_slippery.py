# in this percentile used to define the fraction of episodes to KEEP, starting from the best ones. 
import os, wandb, gymnasium as gym, torch, numpy as np, random
import typing as tt, torch.nn as nn, torch.optim as optim
...
def iterate_batches(env: gym.Env, net: Net, batch_size: int) -> tt.Generator[tt.List[Episode], None, None]:
    ...
    episode_steps = []
    obs, _ = env.reset()
    sm = nn.Softmax(dim=1)   # used in line 47

    while True:
        obs_v = torch.tensor(obs, dtype=torch.float32).unsqueeze(0)
        act_probs_v = sm(net(obs_v))
        act_probs = act_probs_v.detach().numpy()[0]
        action = np.random.choice(len(act_probs), p=act_probs) # this is cross entropy
...
if __name__ == '__main__':
    ...
    wandb.init(project="frozenlake_rl", name="cross_entropy_tweaked_non_slippery")
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        wandb.log({"loss": loss_v.item(), "iter_no": iter_no})
        wandb.log({"reward_mean": reward_mean, "iter_no": iter_no})
        wandb.log({"reward_bound": reward_bound, "iter_no": iter_no})
